...
function StepThree(props){
  ...
  let { image_url, property_name, address, city, state, zipcode, morgage, rent } = props

  function addHouse(){
    Axios.post('/api/house', { image_url, property_name, address, city, state, zipcode, morgage, rent })
      .then(response => {
  ...
  return (
    <div className="step-three step">
      ...
      <input onChange={e => props.updateMorgage(e.target.value)} type="text" value={morgage} />

      <h3>Desired Monthly Rent</h3>
      <input onChange={e => props.updateRent(e.target.value)} type="text" value={rent} />
...
function mapStateToProps(reduxState){
  let { image_url, property_name, address, city, state, zipcode, morgage, rent } = reduxState;
  return {
    image_url,
    property_name,
    address,
